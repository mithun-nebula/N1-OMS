...
- *Provider chosen — Google Meet, live since 2026-08-25.* Created through the Calendar API (`events.insert` with `conferenceData.createRequest`), not the Meet REST API: one call returns the link, creates the event **and emails the invitations**. Authenticated by **OAuth with one shared refresh token**, not a service account — a service account acting on somebody's behalf needs domain-wide delegation, which needs Workspace, and the account here is personal Gmail. `ORG_VIDEO_PROVIDER` selects it; `stub` remains the default so no test can reach Google. **Google sends the external invitations, so no `EmailProvider` is required for meetings** — there is no mail transport anywhere in this codebase and this route means none is needed. One consequence: in Google's eyes every meeting is organised by the single consenting account, though the app's own `organizer` field stays truthful.
